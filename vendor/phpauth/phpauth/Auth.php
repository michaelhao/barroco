...
class Auth
{
	...
	*/
	...
	public function login($acc, $password, $remember = 0, $captcha = NULL)
	{
		$return['error'] = true;
		...
		// $validateEmail = $this->validateEmail($acc);
		...
		$uid = $this->getUID(strtolower($acc));
	...
	*/
	...
	public function register($account, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL)
	{
		$return['error'] = true;
  ...
  //       $zxcvbn = new Zxcvbn();
		...
		if ($this->isAccountTaken($account)) {
			$this->addAttempt();

			$return['message'] = '帳號已經重複';
			return $return;
		}

		// $addUser = $this->addUser($email, $password, $params, $sendmail);
		$addUser = $this->addUserAccount($account, $password, $params, $sendmail);
	...
	*/
	...
	public function getUID($acc)
	{
		$query = $this->dbh->prepare("SELECT id FROM {$this->config->table_users} WHERE account = ?");
		$query->execute(array($acc));
	...
	*/
	...
	/**
	* Checks if an Phone is already in use
	* @param string $email
	* @return boolean
	*/

	public function isAccountTaken($account)
	{
		$query = $this->dbh->prepare("SELECT count(*) FROM {$this->config->table_users} WHERE account = ?");
	...
	*/
	...
	private function addUserAccount($account, $password, $params = array(), &$sendmail)
	{
		$return['error'] = true;
		...
		$uid = $this->dbh->lastInsertId();
		$account = htmlentities(strtolower($account));

		if($sendmail) {
			$addRequest = $this->addRequest($uid, $account, "activation", $sendmail);
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET account = ?, password = ?, isactive = ? {$setParams} WHERE id = ?");

		$bindParams = array_values(array_merge(array($account, $password, $isactive), $params, array($uid)));
	...
	*/
	...
	private function addRequest($uid, $email, $type, &$sendmail)
	{
		...
		if($sendmail === true)
        {
			...
			$mail->CharSet = "utf-8";
			$mail->From = $this->config->smtp_username;
			...
			if($type == "activation") {
				...
				$mail->Body = sprintf($this->lang['email_reset_body'], $this->site_url('site/reset_password')."?key=".$key, '', '');
				$mail->AltBody = sprintf($this->lang['email_reset_altbody'], $this->site_url('site/reset_password')."?key=".$key, '', '');
			}
