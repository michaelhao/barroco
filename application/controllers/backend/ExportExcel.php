...
class ExportExcel extends CI_Controller
{
	public function orderExcel(){
		...
		if ($storeAcc==0) {
			$file_name="所有";
			$orders=$this->db->order_by('id', 'desc')->where('created_at >=', $from)->where('created_at <=', $to)->get_where('order', array())->result_array();	
		}else{
			list($IDstore, $IDacc)=explode("-", $storeAcc);//分店-店員
			if ($IDstore==0) {
				$file_name="線上";
				$orders=$this->db->order_by('id', 'desc')->where('created_at >=', $from)->where('created_at <=', $to)->get_where('order', array('store_from'=>0))->result_array();	
			}else{
				$store=$this->db->get_where('store', array('id'=>$IDstore, 'recover'=>0))->row_array();
				if ($IDacc==0) {
					$file_name=$store['name'];
					$orders=$this->db->order_by('id', 'desc')->where('created_at >=', $from)->where('created_at <=', $to)->get_where('order', array('store_from'=>$IDstore))->result_array();	
				}else{
					$clerk=$this->db->get_where('store_account', array('id'=>$IDacc, 'recover'=>0))->row_array();
					$file_name=$store['name']."_".$clerk['name'];
					$orders=$this->db->order_by('id', 'desc')->where('created_at >=', $from)->where('created_at <=', $to)->get_where('order', array('store_from'=>$IDstore, 'store_clerk'=>$IDacc))->result_array();	
				}
			}
		}
		...
		foreach ($orders as $key => $order) {
			...
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row ,$order['order_payclass']);//實體店家要顯示什麼?
			//下單分店
	...
	public function memberExcel(){
		...
		//在欄位A1 寫入文字
		// $PHPExcel->getActiveSheet()->setCellValue('A1', '測試');
		$titleWrite = array('會員編號' , '姓名' , '手機' , '身分證字號' , '地址' , '生日' , '會員身分' , '購物金' , '紅利點' , '註冊來源' , '註冊時間' , '會員狀態');
		for ($i=0; $i < 12; $i++) { 
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i, 1, $titleWrite[$i]);
		}

		$users=$this->db->order_by('id', 'desc')->get_where('users', array())->result_array();
		$row=2;
		foreach ($users as $key => $user) {
			
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row ,$user['id']);
			
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row ,$user['name']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row ,$user['phone']);
			
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row ,$user['identity']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row ,$user['address']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row ,$user['birthday']);
			...
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row ,$level);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row ,$user['gold']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row ,$user['bonus']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row ,$user['register_from']);
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row ,$user['dt']);
			...
			$PHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row ,$isactive);
