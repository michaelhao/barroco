...
class Question_option extends CI_Controller {
	...
	public function delete_detail() {
		...
		// 導回原本的頁面
		$question_option =$this->db->get_where('question_option', array('id' => $this->input->get("id"),))->row_array();
		$question =$this->db->get_where('question', array('question_id' => $question_option['question_id'],))->row_array();
		$panel= 21;
		$row=select_submenu($panel);
		redirect($row["modifylink"]."&mpanel=".$panel."&id=".$question['id'], 'refresh');
	}
